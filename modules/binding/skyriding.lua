...
local ABILITIES = {
  { key = '1', spellID = 372608 }, -- Surge Forward
  { key = '2', spellID = 372610 }, -- Skyward Ascent
  { key = '3', spellID = 361584 }, -- Whirling Surge
  { key = '4', spellID = 403092 }, -- Aerial Halt
  { key = '5', spellID = 425782 }, -- Second Wind
}
  ...
  for _, ability in next, ABILITIES do
    if IsSpellKnown(ability.spellID) and not ability.button then
      ...
      local button = addon:CreateButton(
        'Button',
        addonName .. 'MountAbilityButton' .. ability.key,
        nil,
